...
Let's create a package called **decorators**.
Inside this pacage we add our code.
...
```
Now let's create some classes for our demonstration.
We start with a class called **FileDataSource**:

**FileDataSource** is extended with the **DataSource** *interface*.
```java
import java.io.*;
...
```
Now let's create another class that also *extends* **DataSourceDecorator**.
We call this class **CompressionDecorator**.
```java
